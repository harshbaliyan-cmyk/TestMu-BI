...
export const OPP_SCHEMA = {
  ...
  owner:            { type: 'string',  group: 'essential', label: 'Owner Name', hint: 'AE/AM rep performance tab, global filter',
                      ...
                      aliases: ['owner', 'ownername', 'fullname', 'rep', 'salesrep', 'accountexecutive', 'opportunityowner'] },
  ...
  createdDate:      { type: 'date',    group: 'essential', label: 'Created date', hint: 'Pipeline created chart, date filter',
                      ...
                      // "Opp Created Date" beats a product line's own "Created Date".
                      preferredHeaders: ['opportunitycreateddate', 'oppcreateddate'],
  ...
  orgType:          { type: 'string',  group: 'segmentation', label: 'Org type', hint: 'Global filter, heatmap, stale thresholds',
                      ...
                      // 'tier' is gone: it fuzzy-matched "Tier Based Pricing?" on the Opp + Product source.
                      aliases: ['orgtype', 'organisationtype', 'organizationtype', 'segment', 'customersegment', 'accountsegment'] },
  ...
  pod:              { type: 'string',  group: 'segmentation', label: 'POD', hint: 'Global filter, team performance',
                      ...
                      // The Opp + Product source carries BOTH "POD" and "Sales POD";
                      // the bare column is this field, the other is salesPod.
                      preferredHeaders: ['pod', 'podname'],
  ...
  product:          { type: 'string',  group: 'segmentation', label: 'Product', hint: 'Global filter, product portfolio bubble',
                      ...
                      // The Opp + Product source lists "Product Family" before "Product Name";
                      // the raw SKU column must win that tie.
                      preferredHeaders: ['productname'],
  ...
  ownerActive:      { type: 'boolean', group: 'segmentation', label: 'Rep is active', hint: 'Rep status filter on every board',
                      ...
                      // "User Active" is this field even when a bare "Active"
                      // (the opportunity's own flag, oppActive) sits beside it.
                      preferredHeaders: ['useractive', 'repactive', 'owneractive'],
  ...
  // ---- Executive Dashboard fields (the "Opp + Product" source) ----
  userId:           { type: 'string',  group: 'other', label: 'User ID', hint: 'Executive Dashboard — quota per user',
                      desc: 'The owning user\'s stable ID. Quota is one number per USER and is read with MIN() per User ID before summing, exactly as {FIXED [User ID]: MIN([Quota])}. Leave unmapped and the rep name is the key instead, which only goes wrong when two reps share a name.',
                      preferredHeaders: ['userid'],
                      aliases: ['userid', 'owneruserid', 'ownerid', 'repid', 'userkey'] },
  misRequired:      { type: 'boolean', group: 'other', label: 'MIS required', hint: 'Executive Dashboard — global filter',
                      desc: 'Whether the opportunity is flagged as needing an MIS review. A global filter on the Executive Dashboard; default All.',
                      preferredHeaders: ['misrequired'],
                      aliases: ['misrequired', 'mis', 'misreq', 'misflag'] },
  oppActive:        { type: 'boolean', group: 'other', label: 'Opportunity is active', hint: 'Executive Dashboard — Active Trials by POD',
                      desc: 'The OPPORTUNITY\'s own Active flag, distinct from "Rep is active" (the owner\'s User Active). The Active Trials by POD chart keeps only active opportunities of active users. On a source that carries both, "User Active" goes to Rep is active and the bare "Active" column belongs here.',
                      aliases: ['active', 'opportunityactive', 'oppactive', 'isactive', 'activeopportunity'] },
  salesPod:         { type: 'string',  group: 'segmentation', label: 'Sales POD', hint: 'Executive Dashboard — segment selector',
                      desc: 'The sales POD name as published on the source (distinct from the display POD used for the per-POD charts). One of the Segment Selector\'s choices on the Executive Dashboard.',
                      aliases: ['salespod', 'salespodname'] },
  lineDuration:     { type: 'number',  group: 'metrics', label: 'Product line duration', hint: 'Executive Dashboard — Product ARR input',
                      desc: 'The product LINE\'s subscription duration in months ("Subscription Duration-1" on the Opp + Product source), separate from the opportunity-level Subscription duration. Product ARR = Total price ÷ this × 12. Leave unmapped on a source with a single duration column and the opportunity-level one is used.',
                      preferredHeaders: ['subscriptionduration1'],
                      aliases: ['subscriptionduration1', 'lineduration', 'productduration', 'linesubscriptionduration', 'productsubscriptionduration'] },
};
...
export const DASHBOARD_FIELD_SETS = {
  ...
  // Executive Dashboard: the "Opp + Product" source at opportunity × product
  // line grain. Both durations are asked for because ARR (opportunity) and
  // Product ARR (line) divide by different columns; `product` must be the RAW
  // SKU column, and the current-quarter quota is mapped by hand.
  executive:   ['id','name','account','owner','userId','ownerActive','oppActive','stage','isClosed','isWon','type','opportunityForecast','amount','subscriptionDuration','closeDate','createdDate','misRequired','product','totalPrice','lineDuration','pod','salesPod','quotaCurrent','orgType','employees','freeDomain','continentGroup'],
};
...
export function applyMapping(rawRows, fieldMapping) {
  ...
  return rawRows.map((raw, i) => {
    ...
    // The Executive Dashboard keys its own product tables on the raw SKU,
    // which the rename below would otherwise erase.
    row.productRaw = rawProductName || null;
