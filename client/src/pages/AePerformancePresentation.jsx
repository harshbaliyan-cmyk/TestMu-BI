...
export default function AePerformancePresentation({ share = false } = {}) {
  ...
  return <HideableProvider value={hideControl}><main className="presentation-shell win-board-wrap win-board-tv-shell">
    ...
    <div className="ae-top-tiles ae-top-tiles-tv">
      <Hideable k="kpi:top-rep" label="Top AE performer"><div className="ae-top-tile">
    ...
    <div className="presentation-slide ae-performance-slide ae-podium-slide">
      <PresentCard hideKey="rep-leaderboard" title="AE Quota Attainment" subtitle={`Won ARR closed in ${quarterLabel} ÷ each rep's quota · ${measured.length} of ${reps.length} carry a target`}>
        <RepLeaderboard reps={reps} comparisons={groupComparisons.reps} topN={repTopN} badges />
      </PresentCard>
      <PresentCard hideKey="pod-leaderboard" title="AE POD Quota Attainment" subtitle={`A POD's quota is the sum of its reps' targets · ${quarterLabel}`}>
    ...
    <footer className={`presentation-controls${isFullscreen && !controlsVisible ? ' controls-hidden' : ''}`}>
      ...
      {!share && <button onClick={() => navigate('/dashboard/ae-performance')}>Exit</button>}
    </footer>
