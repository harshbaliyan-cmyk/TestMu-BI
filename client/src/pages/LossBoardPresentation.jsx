...
export default function LossBoardPresentation() {
  ...
  return <main className="presentation-shell win-board-wrap win-board-tv-shell">
    ...
    <div className="presentation-slide win-board-tv-layout">
      <section className="win-board-tv-main" aria-label="Loss Board charts and KPIs">
        ...
        <div className="win-board-tv-chart-grid">
          <PresentCard title="Lost after trial" subtitle="Of the closed opportunities that reached a trial, the share that were lost">
          ...
          <PresentCard title={`Top loss reasons by ${definition.label}`}>
            {reasonTopN === 5
      ...
      <aside className="win-board-tv-pod-rail" aria-label={`${definition.label} by POD`}>
        <PresentCard title={`${definition.label} by POD`} subtitle={`${timeRangeSummary} · Top 5 · gauge = current, dot = previous`}>
    ...
    <footer className={`presentation-controls${isFullscreen && !controlsVisible ? ' controls-hidden' : ''}`}>
      ...
      <button onClick={() => navigate('/dashboard/loss-board')}>Exit</button>
    </footer>
  </main>;
}
